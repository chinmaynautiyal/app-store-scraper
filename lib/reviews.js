...
function cleanList (results) {
  ...
  return reviews.map((review) => ({
    String('id'): review.id.label,
    String('userName'): review.author.name.label,
    String('userUrl'): review.author.uri.label,
    String('version'): review['im:version'].label,
    String('score'): parseInt(review['im:rating'].label),
    String('title'): review.title.label,
    String('text'): review.content.label,
    String('url'): review.link.attributes.href
  }));
}
